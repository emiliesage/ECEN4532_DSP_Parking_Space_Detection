...
STD_THRESHOLD = 0.23; %0.24 worked ok
...
blurred = imgaussfilt(double(mf.(video_name)(:,:,1)), GAUSS_SIGMA);
for s = 1:n_spaces
...
for batch_id = 1:total_batches
    ...
    for local_f = 1:n_batch
        ...
        for s = 1:n_spaces
            new_statuses(s) = applySTD(blurred, bounds(s,:), masks{s}, ...
                                            STD_THRESHOLD, image_height, image_width,mean_init(s));
            roi = genRoi(blurred,bounds(s,:),masks{s},image_height,image_width);
            m(s) = mean(roi(masks{s}));  
        
            if new_statuses(s)  % free — update reference toward current
                %mean_init(s) = alpha * m(s) + (1-alpha) * mean_init(s);
                mean_init(s) = m(s);
            % occupied — do nothing, freeze the reference
            end           
...
function is_free = applySTD(blurred, rect, mask, threshold, H, W, mean_init)
    ...
    mean_drop = (mean_init - current_mean) / mean_init;  % how much darker vs reference
    
    is_free = std_ratio < threshold && mean_drop < 0.25;
    % occupied if STD is high OR mean dropped >25% from reference
end
...
s_dev = sqrt(sum((data - mean).^2) / length(data));
